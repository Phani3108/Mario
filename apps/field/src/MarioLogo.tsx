...
/**
 * Mario wordmark + lockup — field-app copy.
 * Kept identical to apps/web/src/components/MarioLogo.tsx to avoid pulling
 * the web bundle into Vite. If we ever extract a shared UI package, fold both.
 */

type Tone = 'dark' | 'light';

export function MarioMark({
  size = 36,
  tone = 'dark',
  className = '',
  title = 'Mario',
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" role="img" aria-label={title} className={className}>
      <defs>
        <linearGradient id={`mario-bg-${id}`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={bg} />
          <stop offset="100%" stopColor={tone === 'dark' ? '#1E293B' : '#F1F5F9'} />
        </linearGradient>
        <linearGradient id={`mario-tower-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={accent} />
          <stop offset="100%" stopColor={accent2} />
        </linearGradient>
      </defs>
      <rect x="2" y="2" width="60" height="60" rx="14" fill={`url(#mario-bg-${id})`} />
